...
export const signUp = validatedAction(signUpSchema, async (data, formData) => {
  ...
  if (!role) {
    return { error: 'Failed to create user roles. Please try again.' }
  }
  ...
  if (inviteId && email) {
    ...
    const _invitationsRolesConnection = await db
      .select()
      .from(invitationsRolesConnections)
      .where(and(eq(invitationsRolesConnections.invitationsId, parseInt(inviteId))))

    if (invitation) {
      teamId = invitation.teamId
      const memberRoles = _invitationsRolesConnection.map((item) => {
        return item.teamsMemberRoleId
      })
  ...
  await Promise.all([
    ...
    db.insert(teamMembersRolesConnections).values({
      teamId: teamId,
      userId: createdUser.id,
      teamMemberRoleId: 1,
    }),
